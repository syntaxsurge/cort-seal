...
- Wallet identity uses RainbowKit + wagmi providers in `src/app/providers.tsx` with an injected MetaMask connector and Arbitrum Sepolia as the supported chain, and authenticated UI gating lives in `src/components/auth/RequireWallet.tsx` with wallet capture via hidden inputs.
